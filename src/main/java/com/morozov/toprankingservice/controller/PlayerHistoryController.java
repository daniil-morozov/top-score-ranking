...
@RestController
@RequestMapping(path = "/playerscorehistory", produces = MediaType.APPLICATION_JSON_VALUE)
public class PlayerHistoryController {
  ...
  public ResponseEntity<List<PlayerHistoryResponse>> getAll(@PathVariable String player) {
    List<PlayerHistoryResponse> result =
        playerScoreHistoryService.getAll(player.toLowerCase(Locale.ROOT)).stream()
  ...
  public ResponseEntity<PlayerHistoryResponse> getTop(@PathVariable String player) {
    PlayerHistoryResponse result =
        new PlayerHistoryResponse(
            playerScoreHistoryService.getTop(player.toLowerCase(Locale.ROOT)));
  ...
  public ResponseEntity<PlayerHistoryResponse> getLowest(@PathVariable String player) {
    PlayerHistoryResponse result =
        new PlayerHistoryResponse(
            playerScoreHistoryService.getLowest(player.toLowerCase(Locale.ROOT)));
  ...
  public ResponseEntity<Double> getAverage(@PathVariable String player) {
    Double result = playerScoreHistoryService.getAverage(player.toLowerCase(Locale.ROOT));
