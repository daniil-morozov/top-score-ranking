...
public interface ScoreRepository extends PagingAndSortingRepository<ScoreEntity, UUID> {
  ...
  @Query(
      value = "select s.time as time, s.score as score from score s where s.player = ?1",
      nativeQuery = true)
  List<TimeScore> findAllByPlayer(String player);

  @Query(
      value =
          "select s1.time as time, score from score s1 "
              + "where score=(select max(s2.score) from score s2 where s2.player = s1.player) "
              + "and player= ?1 limit 1",
      nativeQuery = true)
  ...
  Page<ScoreEntity> findByPlayerInAndTimeBetween(
      List<String> player, LocalDateTime after, LocalDateTime before, Pageable pageable);

  Page<ScoreEntity> findByPlayerInAndTimeAfter(
      List<String> player, LocalDateTime after, Pageable pageable);

  Page<ScoreEntity> findByPlayerInAndTimeBefore(
      List<String> player, LocalDateTime before, Pageable pageable);

  Page<ScoreEntity> findByTimeBetween(LocalDateTime after, LocalDateTime before, Pageable pageable);
